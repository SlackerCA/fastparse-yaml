...
object YamlParser extends YamlParser(0,???) {
  private def apply(val indentation:Int, val context:Context): YamlParser = new YamlParser(indentation,context)
...
*/
  ...
  val s_double_escaped = s_white.rep ~ "\" ~ b_non_content ~ Pass.flatMap(YamlParser(indentation,FlowIn).l_empty.rep) ~ s_flow_line_prefix(n)
  ...
  val nb_single_text = P(Pass.flatMap(unit => this.context match {
    case BlockOut => s-separate-lines
    case BlockIn  => s-separate-lines
    case FlowOut  => s-separate-lines
    case FlowIn   => s-separate-lines
  ...
  val ns_plain_safe = P(Pass.flatMap(unit => this.context match {
  ...
  //                                               c = flow-key  ⇒ flow-key
  val in_flow = this.context match {
    case FlowOut  => FlowIn
    case FlowIn   => FlowIn
  ...
  val c_flow_sequence = "[" ~ s_separate.? ~ Pass.flatMap(YamlParser(indentation,in_flow).ns_s_flow_seq_entries).? ~ "]"
  ...
  val c_flow_mapping = "{" ~ s_separate.? ~ Pass.flatMap(YamlParser(indentation,in_flow).ns_s_flow_map_entries).? ~ "}"
  ...
  val ns_flow_pair_yaml_key_entry = ns_s_implicit_yaml_key(FlowKey) ~ c_ns_flow_map_separate_value
  //[153]   c-ns-flow-pair-json-key-entry(n,c) ::= c-s-implicit-json-key(flow-key) c-ns-flow-map-adjacent-value(n,c)
  val c_ns_flow_pair_json_key_entry = c_s_implicit_json_key(FlowKey) ~ c_ns_flow_map_adjacent_value
  //TODO: test that ignoring (n/a) is OK
  ...
  //                                               /* Empty */  ⇒ m = auto-detect()
  val c_indentation_indicator:Parser[Integer] = CharIn("123456789").!.map(Integer.parseInt)
...
val c_l_block_seq_entry = "-" /* Not followed by an ns_char */ s_l_block_indented(n,block_in)
...
val s_l_block_indented = ( s_indent(m) ~ ( ns_l_compact_sequence(n+1+m) | ns_l_compact_mapping(n+1+m) ) ) | s_l_block_node | ( e_node ~ s_l_comments )
...
val l_block_mapping = ( s_indent(n+m) ~ ns_l_block_map_entry(n+m) ).rep(1) /* For some fixed auto_detected ~ m > 0 */
...
val c_l_block_map_explicit_key = "?" ~ s_l_block_indented(n,block_out)
//[191]   l-block-map-explicit-value(n)      ::= s-indent(n) ":" s-l+block-indented(n,block-out)
val l_block_map_explicit_value = s_indent ~ ":" ~ s_l_block_indented(n,block_out)
...
val c_l_block_map_implicit_value = ":" ~ ( s_l_block_node(n,block_out) | ( e_node ~ s_l_comments ) )
...
val s_l_flow_in_block = s_separate(n+1,flow_out) ~ ns_flow_node(n+1,flow_out) ~ s_l_comments
...
val s_l_block_scalar = Pass.flatMap(YamlParser(indentation+1,context).s_l_block_scalar_pre)  ~ ( c_l_literal | c_l_folded )
...
val s_l_block_collection = ( s_separate(n+1,c) ~ c_ns_properties(n+1,c) ).? ~ s_l_comments ~ ( l_block_sequence(seq_spaces) | l_block_mapping )
//[201]   seq-spaces(n,c)                    ::= c = block-out ⇒ n-1
val seq_spaces = c = block_out ⇒ n_1
...
val l_bare_document = s_l_block_node(-1,block_in) /* Excluding c_forbidden ~ content */
...
private class BlockScalar(val indentation:Integer, val chomping:Chomping) {
  ...
  val l_keep_empty = (Pass.flatMap(YamlParser(indentation,BlockIn).l_empty.rep) ~ l_trail_comments.?)
  ...
  val b_chomped_empty = chomping match {
    case Strip => l_strip_empty
    case Clip  => l_strip_empty
    case Keep  => l_keep_empty
  }

  val l_empty_rep = Pass.flatMap(YamlParser(indentation,BlockIn).l_empty.rep)
  ...
  val l_nb_folded_lines = s_nb_folded_text ~ ( Pass.flatMap(YamlParser(indentation,BlockIn).b_l_folded) ~ s_nb_folded_text ).rep
